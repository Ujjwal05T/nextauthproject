...
export const sendEmail = async ({ email, emailType, userId }: any) => {
  try {
    ...
    if(emailType==="VERIFY"){
        ...
        await User.findByIdAndUpdate(userId,{$set:{forgotPasswordToken:hashedToken,forgotPasswordTokenExpire:Date.now()+3600000}})
    }
    ...
    const mailOptions = {
      ...
      html: `<p> Click <a href="${process.env.DOMAIN}/verify?token=${hashedToken}">here</a>
